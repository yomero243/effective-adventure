...
// Componente para tu modelo 3D personal
const ModelComponent: React.FC = () => {
  const { scene } = useGLTF('./Template.glb');
  const modelRef = useRef<THREE.Group>(null);
  
  React.useEffect(() => {
    // Escala apropiada para una persona
    scene.scale.set(1, 1, 1);
    
    // Posición centrada en el suelo
    scene.position.set(0, 0, 0);
  }, [scene]);

  // Animación sutil de respiración/movimiento natural
  useFrame((state) => {
    if (modelRef.current) {
      const time = state.clock.getElapsedTime();
      // Movimiento sutil de respiración
      modelRef.current.position.y = Math.sin(time * 0.8) * 0.02;
      // Rotación muy sutil para mostrar diferentes ángulos
...
const CameraControllerComponent: React.FC = () => {
  ...
  useEffect(() => {
    const handleCameraNavigation = (event: Event) => {
      ...
      // Primero zoom out ligeramente
      timeline.to(camera.position, {
        z: camera.position.z + 5,
        duration: 0.3,
        ease: "power1.out"
      })
      // Luego movimiento principal con curva
      .to(camera.position, {
        x: position.x,
        y: position.y,
        z: position.z,
      ...
      // Efecto de "settle" al final
      .to(camera.position, {
        x: position.x + Math.sin(Date.now() * 0.01) * 0.1,
        y: position.y + Math.cos(Date.now() * 0.01) * 0.1,
        duration: 0.5,
